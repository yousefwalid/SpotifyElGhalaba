...
const getCategoryPlaylists = async (categoryId, queryParams) => {
    const features = new ApiFeatures(Category.findById(categoryId).select('playlists'), queryParams).skip();
    const {
        playlists
...
exports.getCategoryPlaylists = catchAsync(async (req, res, next) => {
    const playlists = await getCategoryPlaylists(req.params.category_id, req.query);
