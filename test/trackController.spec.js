...
describe('Testing track controller', function() {
  ...
  it('testing creating a track', async function() {
    ...
    try{
      await trackController.createTrackLogic(track[1],"5e8281b93f83d84d5ab32e51");
    }catch(err){
      assert.strictEqual(err.statusCode,404);
    }
    try{
  ...
  it('testing updating a track with invalid ID',async function(){
    try{
      await trackController.updateTrackLogic("5e8281b93f83d84d5ab32e51",{name:"numb"},"5e8281b93f83d84d5ab32e51");
    }catch(err){
      assert.strictEqual(err.statusCode,404);
    }
  });
  it('testing updating a track with other artist ID',async function(){
    ...
    try{
      await trackController.updateTrackLogic(dbTrack._id,{name:"numb"},createdArtist.userInfo._id);
    }catch(err){
  ...
  it('testing updating a track',async function(){
    const dummyTrack=generateTrack(album._id,user._id);
    const dbTrack=await Track.create(dummyTrack);
    const updated=await trackController.updateTrackLogic(dbTrack._id,{name:"numb"},user.userInfo._id);
  ...
  it('testing updating a track with invalid album update',async function(){
    ...
    const dbTrack=await Track.create(dummyTrack);
    try{
      await trackController.updateTrackLogic(dbTrack._id,{album:"5e8281b93f83d84d5ab32e51"},user.userInfo._id);
    }catch(err){
      assert.strictEqual(err.statusCode,404);
    }
  });
  it('testing updating a track with album update and invalid artist',async function(){
    ...
    try{
      await trackController.updateTrackLogic(dbTrack._id,{album:dummyAlbum._id},user.userInfo._id);
    }catch(err){
      assert.strictEqual(err.statusCode,403);
    }
  });
  it('testing updating a track with album update',async function(){
    ...
    const updated=await trackController.updateTrackLogic(dbTrack._id,{album:dummyAlbum._id},user.userInfo._id);
  ...
  it('testing removing a track with wrong ID',async function(){
    try{
    await trackController.removeTrackLogic("5e8281b93f83d84d5ab32e51",user.userInfo._id);
    }catch(err){
      assert.strictEqual(err.statusCode,404);
    }
  });
  it('testing removing a track with invalid artist',async function(){
    ...
    const createdArtist=await generateArtist();
    try{
    await trackController.removeTrackLogic(dummyTrack._id,createdArtist.userInfo._id);
    }catch(err){
      assert.strictEqual(err.statusCode,403);
    }
  });
